...
# Função para calcular SHA256 e MD5
def calculate_hashes(text):
    sha256 = hashlib.sha256(text.encode('utf-8')).hexdigest()
    md5 = hashlib.md5(text.encode('utf-8')).hexdigest()
    return sha256, md5

# Frases e seus hashes fornecidos
phrases = {
...
for phrase, (expected_sha256, expected_md5) in phrases.items():
    sha256, md5 = calculate_hashes(phrase)
    is_sha256_correct = sha256 == expected_sha256
    is_md5_correct = md5 == expected_md5
    results.append({
        "Frase": phrase,
        "SHA256 Correto": is_sha256_correct,
        "MD5 Correto": is_md5_correct,
        "Resultado": "Verdadeira" if is_sha256_correct and is_md5_correct else "Falsa"
    })

# Exibindo a tabela
for result in results:
    print(f"Frase: {result['Frase']}")
    print(f"SHA256 Correto: {result['SHA256 Correto']}")
    print(f"MD5 Correto: {result['MD5 Correto']}")
